...
| `API_FOOTBALL_KEY` | No | **Recommended for pre-season CI sync.** Same [API-Sports](https://www.api-football.com/) key as repo secret `API_FOOTBALL_KEY` used by GitHub Actions sync. |
| `NEXT_PUBLIC_SITE_URL` | No | Canonical public URL (e.g. `https://www.faleague-ai.com`) for Supabase password-reset emails and CI pre-season cache revalidation. |
| `REVALIDATE_SECRET` | No | Bearer token for `POST /api/revalidate/preseason` (set the same value in GitHub Actions secret `REVALIDATE_SECRET`). |
