...
import type { XaDivergenceRow } from "@/lib/fpl/insights/xa-divergence";
import {
  InsightsSortableTh,
  sortInsightRows,
...
function xaSortValue(row: XaDivergenceRow, key: SortKey): string | number | null {
  switch (key) {
    case "player":
      return row.web_name;
    case "team":
    ...
    case "fplXa":
      return row.fpl_xa;
    case "usXa":
      return row.understat_xa;
    case "fplDelta":
      return row.fpl_xa_vs_actual;
    case "usDelta":
      return row.understat_xa_vs_actual;
    case "fplUs":
    default:
      return row.fpl_xa_vs_understat != null
        ? Math.abs(row.fpl_xa_vs_understat)
        : null;
  }
}

export function XaDivergencePanel({
  rows,
  minMinutes,
  labels,
}: {
  rows: XaDivergenceRow[];
  minMinutes: number;
  labels: {
    intro: string;
    filterPos: string;
  ...
  const filtered = useMemo(() => {
    ...
    return sortInsightRows(list, (row) => xaSortValue(row, sortKey), sortDir);
          ...
          <table className="w-full min-w-[960px] text-left text-sm">
            <thead>
              <tr className="border-b border-border bg-card text-xs uppercase tracking-wider text-muted-foreground">
                ...
                />
                <InsightsSortableTh
                  label={labels.colFplXa}
                  active={sortKey === "fplXa"}
                  dir={sortDir}
                  align="right"
                  onSort={() => toggle("fplXa")}
                />
                <InsightsSortableTh
                  label={labels.colUsXa}
                  active={sortKey === "usXa"}
                  dir={sortDir}
                  align="right"
                  onSort={() => toggle("usXa")}
            ...
            <tbody>
              {filtered.map((row) => (
                ...
                >
                  ...
                  <td className="px-3 py-2 text-right tabular-nums">
                    {row.fpl_xa.toFixed(2)}
                  </td>
                  <td className="px-3 py-2 text-right tabular-nums">
                    {row.understat_xa != null
                      ? row.understat_xa.toFixed(2)
                      : "—"}
                  </td>
                  <td
                    className={`px-3 py-2 text-right tabular-nums font-medium ${deltaClass(row.fpl_xa_vs_actual)}`}
                  >
                    {fmtNum(row.fpl_xa_vs_actual)}
                  </td>
                  <td
                    className={`px-3 py-2 text-right tabular-nums ${deltaClass(row.understat_xa_vs_actual)}`}
                  >
                    {fmtNum(row.understat_xa_vs_actual)}
                  </td>
                  <td
                    className={`px-3 py-2 text-right tabular-nums ${deltaClass(row.fpl_xa_vs_understat)}`}
                  >
                    {fmtNum(row.fpl_xa_vs_understat)}
                  </td>
